...
ENT.JumpSound = Sound("npc_hi5/jump.mp3")
ENT.JumpHighSound = Sound("npc_hi5/spring.mp3")
ENT.TauntSounds = {
	Sound("npc_hi5/pieceofcake.mp3")
}
...
local npc_hi5_acquire_distance =
	CreateConVar("npc_hi5_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which Hi-5 will chase a target.")

local npc_hi5_spawn_protect =
	CreateConVar("npc_hi5_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, Hi-5 will not target players or hide within 200 units of \z
	a spawn point.")

local npc_hi5_attack_distance =
	CreateConVar("npc_hi5_attack_distance", 80, FCVAR_NONE,
	"The reach of Hi-5's attack.")

local npc_hi5_attack_interval =
	CreateConVar("npc_hi5_attack_interval", 0.2, FCVAR_NONE,
	"The delay between Hi-5's attacks.")

local npc_hi5_attack_force =
	CreateConVar("npc_hi5_attack_force", 800, FCVAR_NONE,
	"The physical force of Hi-5's attack. Higher values throw things \z
	farther.")

local npc_hi5_smash_props =
	CreateConVar("npc_hi5_smash_props", 1, FCVAR_NONE,
	"If set to 1, Hi-5 will punch through any props placed in their way.")

local npc_hi5_allow_jump =
	CreateConVar("npc_hi5_allow_jump", 1, FCVAR_NONE,
	"If set to 1, Hi-5 will be able to jump.")

local npc_hi5_hiding_scan_interval =
	CreateConVar("npc_hi5_hiding_scan_interval", 3, FCVAR_NONE,
	"Hi-5 will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant Hi-5s are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_hi5_hiding_repath_interval =
	CreateConVar("npc_hi5_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to Hi-5's hiding spot will be redetermined every X seconds.")

local npc_hi5_chase_repath_interval =
	CreateConVar("npc_hi5_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of Hi-5's target will be redetermined every \z
	X seconds.")

local npc_hi5_expensive_scan_interval =
	CreateConVar("npc_hi5_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_hi5_force_download =
	CreateConVar("npc_hi5_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download Hi-5 resources \z
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_hi5: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other Hi-5s, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "Hi-5MissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_hi5" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why Hi-5 isn't working.
	net.Start("Hi-5_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_hi5: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_hi5: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_hi5: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_hi5: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_hi5: Couldn't add seed %s at %s", seed,
				pos))
		end
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "Hi-5NavGen", navEndGenerate)
	else
		print("npc_hi5: nav_generate failed to initialize")
...
concommand.Add("npc_hi5_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_hi5: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. Hi-5 doesn't use
	...
	elseif isConsole then
		print("npc_hi5: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_hi5_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_hi5_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_hi5_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_hi5_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. Hi-5 means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_hi5_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_hi5_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_hi5_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_hi5_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_hi5_attack_distance:GetInt()
		...
		local repathInterval = npc_hi5_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_hi5_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_hi5_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_hi5_hiding_repath_interval:GetFloat()
...
local lastPanic = 0 -- The last time we were in music range of a Hi-5.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_hi5_music_volume =
	CreateConVar("npc_hi5_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by Hi-5. (0-1, where 0 is muted)")

-- If another Hi-5 comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, Hi-5s do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_Hi-5_PANIC_COUNT Hi-5s are this close,
...
local function updatePanicMusic()
	if #ents.FindByClass("npc_hi5") == 0 then
		...
		timer.Remove("Hi-5PanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_hi5_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_hi5" then
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear Hi-5 taunt us.
...
local function startTimer()
	if not timer.Exists("Hi-5PanicMusicUpdate") then
		timer.Create("Hi-5PanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	...
	-- Hi-5 should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("Hi-5HUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("Hi-5HUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("hi5 is studying this map.", "Hi-5HUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "Hi-5HUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "Hi-5HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "Hi-5HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "Hi-5HUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("Hi-5_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "Hi-5NavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. Hi-5 doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_hi5_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for hi5 to assuty you \z
		this map out.\n\z
...
net.Receive("Hi-5_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! Hi-5 doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet Hi-5-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! Hi-5 doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to Hi-5.\n\z
			\n\z
			If you ARE the server host, you can run npc_hi5_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not Hi-5-compatible!",
			"Ok", nil,
